...
namespace GoogleLibrary.Custom
{
    ...
    public class BasicEvent
    {
        ...
        /// <summary>
        /// Retrieve data as a DateTime.
        /// </summary>
        /// <param name="enumEventFieldType"></param>
        /// <param name="fields"></param>
        /// <param name="data"></param>
        /// <returns></returns>
        protected static DateTime? GetDateTimeOrDefault(EnumEventFieldType enumEventFieldType, List<Tuple<string, EnumEventFieldType>> fields, List<string> data)
        {
            ...
            if (!string.IsNullOrWhiteSpace(value))
                date = DateTimeHelper.Parse(value, CultureInfo.InvariantCulture);
            return date;
        }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        protected static List<T> GetList<T>(EnumEventFieldType enumEventFieldType, List<Tuple<string, EnumEventFieldType>> fields, List<string> data)
        {
            var index = fields.FindIndex(x => x.Item2 == enumEventFieldType);
            if (index < 0)
                return [];
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        protected static string GetString(EnumEventFieldType enumEventFieldType, List<Tuple<string, EnumEventFieldType>> fields, List<string> data)
        {
            var index = fields.FindIndex(x => x.Item2 == enumEventFieldType);
            if (index < 0)
                return "";
        ...
        {
            var isAirportToAirport = from.IsAirportToAirport(to);
            Locations =
            [
                from,
                .. via,
        ...
        /// <summary>
        /// Conversion of EnumEventFieldType to a string representation.
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="enumEventFieldType"></param>
        /// <param name="fields"></param>
        ...
        /// <returns></returns>
        public static T2 GetEnum<T1, T2>(T1 enumType, List<Tuple<string, T1>> fields, List<string> data)
                where T1 : Enum
                where T2 : Enum
        {
            ...
            if (index < 0)
                return default;
            var value = data[index];
            return EnumHelper.StringToEnumOrDefault<T2>(value);
        }
        ...
        {
            Title = GetString(EnumEventFieldType.Summary, fields, data);
            StartDate = GetDateTimeOrDefault(EnumEventFieldType.StartDate, fields, data) ?? DateTime.UtcNow;
            StartTime = GetDateTimeOrDefault(EnumEventFieldType.StartTime, fields, data);
            EndDate = GetDateTimeOrDefault(EnumEventFieldType.EndDate, fields, data);
            EndTime = GetDateTimeOrDefault(EnumEventFieldType.EndTime, fields, data);
            Status = GetEnum<EnumEventFieldType, EventStatus>(EnumEventFieldType.Status, fields, data);
            Category = GetEnum<EnumEventFieldType, EventCategory>(EnumEventFieldType.Category, fields, data);
            Description = GetString(EnumEventFieldType.Description, fields, data);

            Reminders = GetList<int>(EnumEventFieldType.Reminders, fields, data);
            ...
            var from = new Location(GetString(EnumEventFieldType.From, fields, data), GetString(EnumEventFieldType.FromAddress, fields, data));
            var to = new Location(GetString(EnumEventFieldType.To, fields, data), GetString(EnumEventFieldType.ToAddress, fields, data));
            var via1 = new Location(GetString(EnumEventFieldType.Via, fields, data), GetString(EnumEventFieldType.ViaAddress, fields, data));
            var via2 = new Location(GetString(EnumEventFieldType.Via2, fields, data), GetString(EnumEventFieldType.ViaAddress2, fields, data));
        ...
        {
            var list = fields.Where(x => x.Item2.GetAttribute<DescriptionAttribute>() != null);
